...
parser.add_option("--hf", dest="horizontal_flips",
                  help="Augment with horizontal flips in training.", default=True)
...
parser.add_option("--input_weight_path", dest="input_weight_path",
                  help="Input path for weights. If not specified, will try to load default "
                       "weights provided by keras.",
...
with open(config_output_filename, 'w') as config_f:
    ...
    print(
        'Config has been written to {}, and can be loaded when testing to ensure correct '
        'results'.format(config_output_filename))
...
# this is a model that holds both the RPN and the classifier, used to load/save weights for
# the models
...
while True:
    try:
        ...
        if len(rpn_accuracy_rpn_monitor) == epoch_length and C.verbose:
            ...
            rpn_accuracy_rpn_monitor = []
            print(
                'Average number of overlapping bounding boxes from RPN = {} for {} previous '
                'iterations'.format(
        ...
        if iter_num == epoch_length:
            ...
            if C.verbose:
                ...
                print(
                    ...
                    'class_acc,{},elapsed_time,{}'.format(
                            loss_rpn_cls, loss_rpn_regr, loss_class_cls, loss_class_regr,
                            class_acc, time.time() - start_time))
            ...
            if epoch_num == 1 or curr_loss < best_loss:
                if C.verbose:
                    print('Total loss decreased from {} to {}, saving weights'.format(best_loss,
                                                                                      curr_loss))
                best_loss = curr_loss
...
# #              ModelCheckpoint(C.model_path, monitor='val_loss', save_best_only=True,
# verbose=1),
#              ModelCheckpoint(C.model_path, monitor='val_loss', save_best_only=True,
# verbose=1),
#              ReduceLROnPlateau(monitor='loss', factor=0.1, patience=5, min_lr=1e-7,
# verbose=1),
