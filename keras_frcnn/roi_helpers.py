...
def calc_iou(R, img_data, C, class_mapping):
    ...
    for bbox_num, bbox in enumerate(bboxes):
        ...
        gta[bbox_num, 2] = int(
            round(bbox['y1'] * (resized_height / float(height)) / C.rpn_stride))
        gta[bbox_num, 3] = int(
            round(bbox['y2'] * (resized_height / float(height)) / C.rpn_stride))
...
def rpn_to_roi(rpn_layer, regr_layer, C, dim_ordering, use_regr=True, max_boxes=300,
    ...
    return non_max_suppression_fast(all_boxes, all_probs, overlap_thresh=overlap_thresh,
