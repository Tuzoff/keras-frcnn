...
def calcY(C, class_mapping, img_data, width, height, resized_width, resized_height):
                ...
                for jy in xrange(output_height):
                    ...
                    for bbox_num in xrange(num_bboxes):
                        ...
                        curr_iou = iou([gta[bbox_num, 0], gta[bbox_num, 2], gta[bbox_num, 1],
                                        gta[bbox_num, 3]], [x1_anc, y1_anc, x2_anc, y2_anc])
                        ...
                        if curr_iou > best_iou_for_bbox[
                            ...
                            tw = np.log(
                                    (gta[bbox_num, 1] - gta[bbox_num, 0]) / (x2_anc - x1_anc))
                            th = np.log(
                                    (gta[bbox_num, 3] - gta[bbox_num, 2]) / (y2_anc - y1_anc))

                        if img_data['bboxes'][bbox_num]['class'] != 'bg':

                            # all GT boxes should be mapped to an anchor box, so we keep
                            # track of which anchor box was best
                            ...
                            # we set the anchor to positive if the IOU is >0.7 (it does not
                            # matter if there was another better box, it just indicates overlap)
                            if curr_iou > C.rpn_max_overlap:
                                bbox_type = 'pos'
                                num_anchors_for_bbox[bbox_num] += 1
                                # we update the regression layer target if this IOU is the
                                # best for the current (x,y) and anchor position
                                if curr_iou > best_iou_for_loc:
                                    best_iou_for_loc = curr_iou
                                    best_regr = (tx, ty, tw, th)

                            # if the IOU is >0.3 and <0.7, it is ambiguous and no included in
                            #  the objective
    ...
    if len(pos_locs[0]) > num_regions/2:
        ...
        y_is_box_valid[0, pos_locs[0][val_locs], pos_locs[1][val_locs], pos_locs[2][
        val_locs]] = 0
        num_pos = num_regions/2

    if len(neg_locs[0]) + num_pos > num_regions:
        val_locs = random.sample(range(len(neg_locs[0])), len(neg_locs[0]) - num_pos)
        y_is_box_valid[0, neg_locs[0][val_locs], neg_locs[1][val_locs], neg_locs[2][
        val_locs]] = 0
    '''
        ...
        while not_valid_gt:
            ...
            if min_iou < largest_iou <= max_iou:
                ...
                if class_num != num_non_bg_classes:
                    y_class_regr[i,
                    4 * class_num:4 * class_num + 4] = 1  # set value to 1 if the sample is
                    # positive
                    y_class_regr[i,
                    num_non_bg_classes * 4 + 4 * class_num:
                    num_non_bg_classes * 4 + 4 * class_num + 4
                    ] = [tx, ty, tw, th]
                break
    ...
    return np.copy(x_rois), np.copy(y_rpn_cls), np.copy(y_rpn_regr), np.copy(
            y_class_num), np.copy(y_class_regr)
    ...
    while True:
        ...
        for img_data in all_img_data:
            try:
                ...
                yield [np.copy(x_img), np.copy(x_rois)], [np.copy(y_rpn_cls),
                                                          np.copy(y_rpn_regr),
                                                          np.copy(y_class_num),
