...
from keras.layers import Input, Add, Dense, Activation, Flatten, Convolution2D, MaxPooling2D, \
...
def conv_block_td(input_tensor, kernel_size, filters, stage, block, strides=(2, 2),
    ...
    shortcut = TimeDistributed(FixedBatchNormalization(axis=bn_axis), name=bn_name_base + '1')(
            shortcut)
...
def classifier_layers(x, trainable=False):
    # compile times on theano tend to be very high, so we use smaller ROI pooling regions to
    # workaround hence a smaller stride in the region that follows the ROI pool)
