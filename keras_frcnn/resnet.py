...
def identity_block(input_tensor, kernel_size, filters, stage, block, trainable=True):
    ...
    x = Convolution2D(nb_filter1, (1, 1), name=conv_name_base + '2a', trainable=trainable)(
        input_tensor)
...
def identity_block_td(input_tensor, kernel_size, filters, stage, block, trainable=True):
    ...
    x = TimeDistributed(
        Convolution2D(nb_filter1, (1, 1), trainable=trainable, kernel_initializer='normal'),
        name=conv_name_base + '2a')(input_tensor)
    x = TimeDistributed(FixedBatchNormalization(trainable=False, axis=bn_axis),
                        name=bn_name_base + '2a')(x)
    x = Activation('relu')(x)

    x = TimeDistributed(
        Convolution2D(nb_filter2, (kernel_size, kernel_size), trainable=trainable,
                      kernel_initializer='normal', padding='same'), name=conv_name_base + '2b')(
        x)
    x = TimeDistributed(FixedBatchNormalization(trainable=False, axis=bn_axis),
    ...
    x = TimeDistributed(
        Convolution2D(nb_filter3, (1, 1), trainable=trainable, kernel_initializer='normal'),
...
def conv_block(input_tensor, kernel_size, filters, stage, block, strides=(2, 2),
    ...
    x = Convolution2D(nb_filter1, (1, 1), strides=strides, name=conv_name_base + '2a',
    ...
    shortcut = Convolution2D(nb_filter3, (1, 1), strides=strides, name=conv_name_base + '1',
                             trainable=trainable)(input_tensor)
    shortcut = FixedBatchNormalization(trainable=False, axis=bn_axis, name=bn_name_base + '1')(
        shortcut)
...
def conv_block_td(input_tensor, kernel_size, filters, stage, block, strides=(2, 2),
    ...
    x = TimeDistributed(Convolution2D(nb_filter1, (1, 1), strides=strides, trainable=trainable,
                                      kernel_initializer='normal'), name=conv_name_base + '2a')(
        input_tensor)
    x = TimeDistributed(FixedBatchNormalization(trainable=False, axis=bn_axis),
                        name=bn_name_base + '2a')(x)
    x = Activation('relu')(x)
    ...
    shortcut = TimeDistributed(
        Convolution2D(nb_filter3, (1, 1), strides=strides, trainable=trainable,
                      kernel_initializer='normal'), name=conv_name_base + '1')(input_tensor)
    shortcut = TimeDistributed(FixedBatchNormalization(trainable=False, axis=bn_axis),
...
def rpn(base_layers, num_anchors):
    ...
    x_regr = Convolution2D(num_anchors * 4, (1, 1), activation='linear',
                           kernel_initializer='normal', name='rpn_out_regress')(x)
